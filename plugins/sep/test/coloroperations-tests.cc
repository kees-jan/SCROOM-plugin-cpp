...
BOOST_AUTO_TEST_CASE(colorOperations_cache) {
  ...
  data = {};
  std::shared_ptr<ConstTile> ptr(new ConstTile(1, 1, 8, data));
...
BOOST_AUTO_TEST_CASE(colorOperations_reduce) {
  ...
  data = {};
  std::shared_ptr<Tile> tile(new Tile(1, 1, 8, data));
  Scroom::MemoryBlobs::RawPageData::ConstPtr constData;
  constData = {};
  std::shared_ptr<ConstTile> ptr(new ConstTile(1, 1, 8, constData));
